$.get("/content/character_set/", (data) => {
  ...
  $(".list-add-set-button:not(.success)")
    .off("click")
    .click((e) => {
      let target = $(e.target);
      ...
      $.post(
        "/learning/student_character_tag/",
        { character_set_id: parentId },
        (data) => {
          parent.addClass("success");
          target.removeClass("fa-plus").addClass("fa-check").off("click");
          $("#char-sets-container #add-set-button-container").before(`
            ...
            `);
          $(".char-set:not(#add-set-button-container)")
            .off("click")
            .click(charSetClick);
        }
      );
