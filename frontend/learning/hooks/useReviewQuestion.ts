 ...
 */
const useReviewQuestion = (
  url: string,
  completion?: (question: ReviewQuestionData) => void
): ReviewQuestionData | null => {
  ...
  useEffect(() => {
    loadData()
      ...
      .catch((status) => {
        console.error(
          `Error fetching review question, server responded with status ${status}`
        );
      });
  }, [url]);
