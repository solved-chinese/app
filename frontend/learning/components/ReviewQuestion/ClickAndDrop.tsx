...
const ClickAndDrop = (props: Props): JSX.Element => {
  ...
  useEffect(() => {
    if (selected.every((value) => value != null)) onSubmit();
  }, [selected]);
  ...
  const onSubmit = () => {
    if (submitted) return;
    props
      .submitAnswer(selected as string[])
      .then(([response, callbackFn]) => {
        setSubmitted(true);
  ...
  return (
    <div className="question-content">
      <div style={{ width: "100%" }}>
        <Question className="use-chinese">{props.content.question}</Question>
        <Description>{props.content.description}</Description>
        <Description>{props.content.title}</Description>
        <AnswerContainer>{showSelected}</AnswerContainer>
        <ChoicesContainer>{showChoices}</ChoicesContainer>
        <SubmitContainer>
          <button
