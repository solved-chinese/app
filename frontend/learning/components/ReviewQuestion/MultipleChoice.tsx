...
const MultipleChoice = (props: Props): JSX.Element => {
  const [selectedAnswer, setSelectedAnswer] = useState<MCAnswer | undefined>(
    undefined
  );
  const [correctAnswer, setCorrectAnswer] = useState<MCAnswer | undefined>(
    undefined
  );
  ...
  useEffect(() => {
    if (selectedAnswer && !submitted) onSubmit(selectedAnswer);
  }, [selectedAnswer]);
  ...
  return (
    <div className="question-content">
      <div style={{ width: "100%" }}>
        ...
        <ChoicesContainer>{choices}</ChoicesContainer>
        <SubmitContainer>
          <button
