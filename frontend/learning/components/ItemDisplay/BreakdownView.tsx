    ...
    return (
      <CharDefList>
        {definitions.map((elem, i) => {
          return (
            <CharDefListItem key={i} className="use-serifs">
              · {elem}
            </CharDefListItem>
          );
        })}
  ...
  render(): JSX.Element {
    ...
    return (
      <div className="breakdown-container">
        {/* title of toggle */}
        <div
          style={this.props.alwaysDisplay ? { display: "none" } : {}}
          className={"breakdown-toggle " + (this.state.show ? "toggled" : "")}
          onClick={this.toggle}
        >
          <h4>{(this.state.show ? "Close " : "Show me the ") + toggleTitle}</h4>
          <i
            className={
              "fas fa-chevron-down " + (this.state.show ? "inversed" : "")
            }
          ></i>
        </div>
        {/* content of toggle */}
        <div className={"breakdown-content " + (this.state.show ? "show" : "")}>
          {type === "radical"
