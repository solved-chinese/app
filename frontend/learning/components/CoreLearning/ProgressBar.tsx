...
export default class ProgressBar extends React.Component<ProgressBarData> {
    ...
    render(): JSX.Element {
        const total = this.props.mastered + this.props.familiar + this.props.remaining + this.props.bonus;

        const margin = 5;
        const modifier = 100 - 4 * margin;
        const colors: string[] = ['rgb(85, 162, 30)', 'rgb(141, 207, 84)',
            'rgb(212, 210, 210)', 'rgb(255, 195, 0)'];
        const barItemLengthArr: number[] = [
            this.props.mastered, this.props.familiar,
            this.props.remaining, this.props.bonus
        ];
        ...
        return (
            <Body>
                ...
                <TextContainer>
                    ...
                    {this.props.bonus > 0 && <Text style = {textStyle[3]}> {this.props.bonus}  bonus!</Text>}
                </TextContainer>
            </Body>
        );
    }
