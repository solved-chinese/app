...
const AssignmentDisplay = (props: Props): JSX.Element => {
  ...
  const assignment = useLoadAssignment(
    `/learning/api/assignment/${props.qid}`,
    (assignment) => {
  ...
  if (assignment === null) return <>loading assignment</>;
  else return renderAssignment(assignment);
};
