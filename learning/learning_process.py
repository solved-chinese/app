...
class LearningProcess(models.Model):
    ...
    TEST_REVIEW = 70
    FINISH = 80
    CHOICES = [(DECIDE, '_decide'),
               (START_LEARN, '_start_learn'),
               ...
               (FINISH, '_finish')]
    # ends states
    state = FSMIntegerField(CHOICES, default=DECIDE)
    ...
    def _decide(self):
        scs = learning.models.StudentCharacter.of(sc_tags=self.sc_tags)
        sc_to_learn = scs.get_one_to_learn()
        sc_to_review, self.review_field_index = scs.get_all_in_progress()
